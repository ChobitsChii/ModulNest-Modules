...
<div class="mc-fullview">
    ...
    <div class="mc-fullview-header">
        ...
        <div class="alert alert-warning py-2 px-3 mb-0 d-flex align-items-center gap-2" style="font-size:0.83rem;">
            ...
            <button type="button" class="btn btn-sm btn-warning ms-auto" id="mc-btn-trust-sender-full"
                    onclick="allowExternalFull('<?= htmlspecialchars(rawurlencode($message['from_email'] ?? $message['from']), ENT_QUOTES, 'UTF-8') ?>')">
                <i class="bi bi-shield-check me-1"></i> Absender vertrauen &amp; neu laden
            </button>
        </div>
        <script>
        async function allowExternalFull(senderEncoded) {
            const btn = document.getElementById('mc-btn-trust-sender-full');
            if (btn) { btn.disabled = true; btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span> Speichern...'; }
            try {
                const res = await fetch('/mail-client/api/whitelist', {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({ scope_type: 'sender', scope_value: decodeURIComponent(senderEncoded) })
                });
                const data = await res.json();
                if (data && data.ok) {
                    window.location.reload();
                } else {
                    alert('Fehler: ' + (data.error || 'Whitelist konnte nicht aktualisiert werden.'));
                    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-shield-check me-1"></i> Absender vertrauen & neu laden'; }
                }
            } catch (err) {
