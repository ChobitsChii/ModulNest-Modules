...
final class MailClientController
{
    ...
    public function messageFullView(Request $request): Response
    {
        ...
        $senderToCheck = (string) ($messageData['from_email'] ?? $messageData['from']);
        $allowExternal = $this->folderCache->isWhitelisted($userId, $senderToCheck);
    ...
    public function apiMessageBody(Request $request): Response
    {
        ...
        $senderToCheck = (string) ($msgData['from_email'] ?? $msgData['from']);
        $allowExternal = $this->folderCache->isWhitelisted($userId, $senderToCheck);
    ...
    /** GET /mail-client/api/whitelist */
    public function apiWhitelistList(Request $request): Response
    {
        $user = $this->requireUser(true);
        if ($user === null) {
            return $this->jsonError('Nicht angemeldet.', 401);
        ...
        $userId = (int) $user['id'];
        $list   = $this->folderCache->listWhitelistForUser($userId);
        return $this->json(['ok' => true, 'whitelist' => $list]);
    }
    ...
    public function apiWhitelistAdd(Request $request): Response
    {
        ...
        if ($scopeType === 'sender') {
            $scopeValue = \ModulNest\MailClient\Repository\FolderCacheRepository::extractCleanEmail($scopeValue);
            if ($scopeValue === '' || !str_contains($scopeValue, '@')) {
                return $this->jsonError('Ungültige E-Mail-Adresse.', 400);
            }
        } else {
            $scopeValue = \ModulNest\MailClient\Repository\FolderCacheRepository::extractCleanDomain($scopeValue);
            if ($scopeValue === '' || !str_contains($scopeValue, '.')) {
                return $this->jsonError('Ungültige Domain.', 400);
            }
        }
    ...
    /** POST /mail-client/api/whitelist/delete */
    public function apiWhitelistDelete(Request $request): Response
    {
        $user = $this->requireUser(true);
        if ($user === null) {
            return $this->jsonError('Nicht angemeldet.', 401);
        ...
        $userId = (int) $user['id'];
        $id     = (int) ($request->inputRaw('id') ?? 0);
        if ($id <= 0) {
            return $this->jsonError('Ungültige ID.', 400);
        }

        $this->folderCache->removeFromWhitelist($userId, $id);
        $list = $this->folderCache->listWhitelistForUser($userId);
        return $this->json(['ok' => true, 'whitelist' => $list]);
    }
