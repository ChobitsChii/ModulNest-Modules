...
<div class="mc-app">
    ...
    <div class="mc-panes">
        ...
        <div class="mc-main-area" id="mc-main-area">
            ...
            <!-- ─ Unterer Teil: Vorschau (UNTER der Liste) ─ -->
            ...
            <!-- Statuszeile am unteren Rand unter der Mail-Vorschau -->
...
mailClientApp.loadFolders = async function(accountId, triggerEl) {
    ...
    try {
        ...
        function translateFolder(name, special) {
            ...
            if (special === 'drafts' || ['drafts', 'draft', 'entwürfe', 'entwuerfe'].includes(base)) return 'Entwürfe';
            if (special === 'trash' || ['trash', 'deleted', 'papierkorb', 'gelöschte elemente', 'deleted items', 'bin'].includes(base)) return 'Papierkorb';
            if (special === 'spam' || ['spam', 'junk', 'junk e-mail', 'unerwünscht', 'spamverdacht'].includes(base)) return 'Spam';
            if (special === 'archive' || ['archive', 'archiv', 'archives'].includes(base)) return 'Archiv';
            return name;
        }
        
        // Nur den bisher aktiv ausgewählten Ordner erfassen
        var activeFolder = '';
        ...
        // Ordner-Liste rendern + "Alle Ordner laden..." Button wieder anhängen
        ...
        var folderHtml = (data.folders || []).map(function(f) {
            var isActive = (f.name === activeFolder);
            var displayName = f.display_name || translateFolder(f.name, f.special);
        ...
        // Aktualisieren-Button wieder anhängen
        ...
        // Click-Binding neu aufsetzen (nicht für Trigger-Button)
        ...
        // Nach dem Laden den State speichern
