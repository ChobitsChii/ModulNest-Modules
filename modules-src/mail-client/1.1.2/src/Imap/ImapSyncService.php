...
final class ImapSyncService
{
    ...
    {
        try {
            // whereUid('1:*') erfordert mindestens eine Where-Klausel für den IMAP SEARCH-Befehl
            // und selektiert alle UIDs im Bereich 1 bis * (alle Nachrichten)
            $messages = $folder->query()->whereUid('1:*')->leaveUnread()->fetchOrder('asc')->get();
            $uids = [];
