...
mailClientApp.loadFolders = async function(accountId, triggerEl) {
    ...
    try {
        ...
        const expandedFolders = document.querySelectorAll(`.mc-folder-item[data-account="${accountId}"]`)
            .map(function(el) { return el.dataset.folder; }).filter(Boolean);
        
        list.innerHTML = (data.folders || []).map(function(f) {
            var isActive = expandedFolders.indexOf(f.name) !== -1;
        ...
        list.querySelectorAll('.mc-folder-item[data-folder]').forEach(function(el) {
            el.addEventListener('click', async function(e) {
                ...
                // Hintergrund-Sync starten (progressives Laden)
