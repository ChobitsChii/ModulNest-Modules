...
<form method="post" action="/mail-client/compose" id="mc-compose-form">
    ...
    <div class="mb-3">
        ...
        <select id="mc-c-account" name="account_id" class="form-select" required>
            <?php foreach ($accounts as $acc): ?>
            <option value="<?= (int) $acc['id'] ?>"
                    <?= $replyAccountId === (int) $acc['id'] ? 'selected' : '' ?>>
                <?= htmlspecialchars($acc['display_name'], ENT_QUOTES, 'UTF-8') ?>
                &lt;<?= htmlspecialchars($acc['email_address'], ENT_QUOTES, 'UTF-8') ?>&gt;
            </option>
            <?php endforeach; ?>
        </select>
    </div>
