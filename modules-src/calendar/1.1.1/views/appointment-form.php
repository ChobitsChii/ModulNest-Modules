...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Automatische Endzeit-Anpassung bei Startzeit-Änderung
    const startInput = document.getElementById('start_at');
    const endInput = document.getElementById('end_at');
    const allDayChk = document.getElementById('all_day');
    ...
    if (startInput && endInput) {
        ...
        let lastEndDate = endInput.value ? new Date(endInput.value) : null;
        ...
        const onStartChange = () => {
            ...
            // Bisherige Dauer ermitteln (Default: 60 Minuten)
            let durationMs = 60 * 60 * 1000;
            if (lastStartDate && lastEndDate && !isNaN(lastStartDate) && !isNaN(lastEndDate)) {
                const diff = lastEndDate.getTime() - lastStartDate.getTime();
                if (diff > 0) {
                    durationMs = diff;
                }
            }
            ...
            // Wenn Endzeit leer oder vor/gleich neuem Start: Endzeit um bisherige Dauer nach vorne verschieben
            if (!currentEnd || isNaN(currentEnd.getTime()) || currentEnd <= newStart) {
            ...
            lastEndDate = new Date(endInput.value);
        };
        ...
        const onEndChange = () => {
            ...
            if (startInput.value) {
                ...
                if (!isNaN(currentStart.getTime()) && currentEnd <= currentStart) {
                    // Falls Ende vor Start gewählt wird, Endzeit auf Start + 1 Stunde korrigieren
                    const fixedEnd = new Date(currentStart.getTime() + (60 * 60 * 1000));
        ...
        if (allDayChk) {
            allDayChk.addEventListener('change', () => {
                if (!startInput.value) return;
                const sDate = startInput.value.split('T')[0];
                const eDate = endInput.value ? endInput.value.split('T')[0] : sDate;
                if (allDayChk.checked) {
                    startInput.value = sDate + 'T00:00';
                    endInput.value = (eDate || sDate) + 'T23:59';
                } else {
                    startInput.value = sDate + 'T09:00';
                    endInput.value = (eDate || sDate) + 'T10:00';
                }
                endInput.min = startInput.value;
                lastStartDate = new Date(startInput.value);
                lastEndDate = new Date(endInput.value);
            });
        }
        ...
        if (form) {
            form.addEventListener('submit', () => {
                if (startInput.value && endInput.value) {
                    ...
                    if (en <= s) {
                        const fixedEnd = new Date(s.getTime() + (60 * 60 * 1000));
                        endInput.value = formatDateTimeLocal(fixedEnd);
                    }
                }
            });
