...
final class CalendarRepository
{
    ...
    public function findByExternalId(int $userId, string $source, string $externalId): ?CalendarDTO
    {
        $stmt = $this->pdo->prepare(
            "SELECT * FROM `calendars` WHERE `user_id` = ? AND `source` = ? AND `external_id` = ? LIMIT 1"
        );
    ...
    public function touchLastSynced(int $calendarId): void
    {
        $stmt = $this->pdo->prepare(
            "UPDATE `calendars` SET `last_synced_at` = CURRENT_TIMESTAMP WHERE `id` = ?"
        );
