...
final class RepositoryManagerController
{
    ...
    /** @var (callable(): array{updates:int,entdecken:int,installiert:int})|null */
    ...
    public function index(Request $request): Response
    {
        ...
        $counts = is_callable($this->countsProvider)
            ? ($this->countsProvider)()
            : ['updates' => 0, 'entdecken' => 0, 'installiert' => 0];

        return new Response(View::render('@modulnest.repository-manager/admin', [
            'counts' => $counts,
            'title' => 'Repository Manager – Modul-Katalog',
    ...
    private function collectChanges(array $source, Request $request): array
    {
        ...
        if ($request->inputRaw('enabled_submitted') !== null || $request->inputRaw('enabled') !== null) {
